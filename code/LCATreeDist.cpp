...
int LCA(int p, int q){
  if (L[p] < L[q]) // ensure node p is at least as deep as node q
    swap(p, q);
  for (int i = log_num_nodes; i >= 0; i--) // "binary search" for the ancestor of 
    if (L[p] - (1 << i) >= L[q]) // node p situated on the same level as q
      p = A[p][i];

  if (p == q) return p;

  for (int i = log_num_nodes; i >= 0; i--) // "binary search" for the LCA
...
int main(int argc, char* argv[]){
  log_num_nodes = lb(num_nodes); // read num_nodes, the total number of nodes
  for (int i = 0; i < num_nodes; i++) {
    int p; // read p, the parent of node i or -1 if node i is the root
    A[i][0] = p;
